...
public class HelpCommand {
  ...
  private int executeHelp(CommandContext<CommandSourceStack> ctx) {
    ...
    sendHelpLine(
        sender, "pa pinata start <pinata> <loc>", "Start pinata countdown", "partyanimals.start");
    sendHelpLine(
        sender, "pa pinata spawn <pinata> <loc>", "Spawn pinata immediately", "partyanimals.spawn");
    ...
    sendHelpLine(
        ...
        "partyanimals.addlocation");
    sendHelpLine(
        sender,
        "pa pinata spawnpoint remove <name>",
        "Remove saved location",
        "partyanimals.removelocation");

    sendHelpLine(sender, "pa vote check <player>", "Check player votes", "partyanimals.votes");
    sendHelpLine(
        sender, "pa vote <add|remove|set>", "Modify vote data", "partyanimals.votes.admin");

    return Command.SINGLE_SUCCESS;
  }

  private void sendHelpLine(
      CommandSender sender, String command, String description, String permission) {
