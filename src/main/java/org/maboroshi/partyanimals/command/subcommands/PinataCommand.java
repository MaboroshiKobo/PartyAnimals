...
public class PinataCommand {
  ...
  private LiteralArgumentBuilder<CommandSourceStack> buildStart() {
    return Commands.literal("start")
        ...
        .then(
            Commands.argument("pinata", StringArgumentType.word())
                .suggests(this::suggestPinataTemplates)
                .executes(
                    ctx ->
                        handleStart(
                            ctx.getSource(), null, StringArgumentType.getString(ctx, "pinata")))
                .then(
    ...
    return Commands.literal("killall")
        ...
        .executes(
            ctx -> {
              ...
              messageHandler.send(
                  ctx.getSource().getSender(), "<prefix> <green>Killed all active pinatas.");
  ...
  private int handleStart(CommandSourceStack source, String locationName, String templateId) {
    ...
    messageHandler.send(
        source.getSender(), config.getMessageConfig().pinata.events.countdownStarted);
    return Command.SINGLE_SUCCESS;
  }
  ...
  private Location resolveLocation(
    ...
    sendUsage(
        source.getSender(), "/partyanimals pinata " + commandContext + " [template] [location]");
    return null;
  }
