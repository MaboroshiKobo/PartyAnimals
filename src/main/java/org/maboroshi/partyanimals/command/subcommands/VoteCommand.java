...
public class VoteCommand {
    ...
    Bukkit.getScheduler()
        .runTaskAsynchronously(
            ...
            () -> {
              ...
              messageHandler.send(
                  sender, "<prefix> <green>Updated votes for <white>" + targetName + "</white>.");
              messageHandler.send(
                  sender,
                  "<prefix> <gray>Old: <yellow>"
                      + currentVotes
                      + "</yellow> -> New: <aqua>"
                      + newTotal
                      + "</aqua>");
  ...
  private int handleTestSafe(CommandContext<CommandSourceStack> ctx, String serviceName) {
    String targetName = StringArgumentType.getString(ctx, "player");
    VoteTester.triggerTestVote(
        ctx.getSource().getSender(), targetName, serviceName, messageHandler);
    return Command.SINGLE_SUCCESS;
  }
}
