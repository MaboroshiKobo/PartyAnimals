...
public class PinataListener implements Listener {
  ...
  public void onPinataHit(EntityDamageByEntityEvent event) {
    ...
    if (!isItemAllowed(player, pinataConfig.interaction.allowedItems)) {
      log.debug(
          "Player "
              + player.getName()
              + " attempted to hit pinata "
              + pinata
              + " with disallowed item.");
      event.setCancelled(true);
      return;
    }
  ...
  private boolean isItemAllowed(Player player, ItemWhitelist allowedItems) {
    if (!allowedItems.enabled
        || allowedItems.materialNames == null
        || allowedItems.materialNames.isEmpty()) {
      return true;
    }
    ...
    if (!isAllowed) {
      ...
      if (allowedItemsMessage != null && !allowedItemsMessage.isEmpty()) {
        messageHandler.send(
            player, allowedItemsMessage, messageHandler.tag("item", heldMaterial.name()));
      }
      return false;
    }
  ...
  private void handlePinataDeath(
      LivingEntity pinata, Player player, PinataConfiguration pinataConfig) {
    log.debug(
        "Handling pinata death for pinata: "
            + pinata
            + " (UUID: "
            + pinata.getUniqueId()
            + ") by player: "
    ...
    messageHandler.send(
        plugin.getServer(), downedMessage, messageHandler.tag("player", player.getName()));
